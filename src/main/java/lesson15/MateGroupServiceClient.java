...
public class MateGroupServiceClient {
    ...
    private static HttpURLConnection addStudent(String pathParam) throws MalformedURLException, IOException, ProtocolException {
        URL url = new URL("http://localhost:10000/rs/mate/" + pathParam);
    ...
    private static HttpURLConnection getMateGroup() throws MalformedURLException, IOException, ProtocolException {
        URL url = new URL("http://localhost:10000/rs/mate/18122018");
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
    ...
    private static HttpURLConnection removeStudent(String pathParam) throws MalformedURLException, IOException, ProtocolException {
        URL url = new URL("http://localhost:10000/rs/mate/" + pathParam);
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setDoOutput(true);
        conn.setRequestMethod("DELETE");
        conn.setRequestProperty("Content-Type", "application/json");
        String input = "{\"groupId\": \"1\",\"surname\": Rabinovish	}";
